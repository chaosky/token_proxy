...
pub(crate) const USER_AGENT: &str = "codex_cli_rs/0.144.1 (token_proxy)";

const MINIMUM_CLIENT_VERSION: [u64; 3] = [0, 144, 0];

/// 仅提升调用方已经携带的低版本，不要求普通代理请求新增 `version` 头。
pub(crate) fn enforce_minimum_client_version(version: &str) -> &str {
...
pub(crate) fn supported_official_user_agent(user_agent: &str) -> Option<&str> {
...
pub(crate) fn official_originator_from_user_agent(user_agent: &str) -> Option<&str> {
...
pub(crate) fn is_official_originator(originator: &str) -> bool {
